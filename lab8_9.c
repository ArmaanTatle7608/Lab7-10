...
unsigned char CPU_Decode(unsigned int machineCode){
    ...
    // If opcode is 0, I can treat it as an R-type instruction and return the function field instead
    if (opcode == 0){
...
void CPU_Execution(unsigned char opcode, unsigned int machineCode, char *mem){
    ...
    switch (opcode)
    {
        ...
        case 0x20:   // add or lb
            if (realOpcode == 0){   // add (R-type, funct = 0x20)
                ...
                regFile[rt] = (int)read_byte(mem, regFile[rs] + imm);
            }
            break;
        ...
        case 0x23:   // lw
            ...
            rt = (machineCode >> 16) & 0x1F;
            imm = (short)(machineCode & 0xFFFF);
            regFile[rt] = (int)read_dword(mem, regFile[rs] + imm);
            break;
        ...
        case 0x2B:   // sw
            ...
            rt = (machineCode >> 16) & 0x1F;
            imm = (short)(machineCode & 0xFFFF);
            write_dword(mem, regFile[rs] + imm, (unsigned int)regFile[rt]);
            break;
